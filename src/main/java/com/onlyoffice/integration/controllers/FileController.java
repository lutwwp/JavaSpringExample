...
public class FileController {
    ...
    public String convert(@RequestBody Converter body,  // convert a file
        ...
        try{
            if(fileUtility.getConvertExts().contains(fileExt)){  // check if the file with such an extension can be converted
                ...
                storageMutator.createFile(FileSystems.getDefault().getPath(storagePathBuilder.getFileLocation(correctedName)), stream);
    ...
    public String create(@RequestParam("fileExt") String fileExt,  // create a sample file of the specified extension
        ...
        if(fileExt != null){
            try{
                ...
                return "redirect:editor?fileName=" + URLEncoder.encode(fileName, "UTF_8");  // redirect the request
            }catch (Exception ex){
    ...
    {
        String fileName = FileSystems.getDefault().getPath("assets", "sample", fileUtility.getFileName(name)).toString();
    ...
    {
        String fileName = FileSystems.getDefault().getPath("assets", "sample", "csv.csv").toString();
        return downloadFile(fileName);
    }
